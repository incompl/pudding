...
function captureWindow(winId) {
  const file = path.join(os.tmpdir(), `caliper-${process.pid}-${captureSeq++}.png`);
  try {
    execFileSync("screencapture", ["-x", "-o", "-l", String(winId), file], {
      stdio: ["ignore", "ignore", "pipe"],
    });
  } catch (e) {
    const err = String(e.stderr ?? e.message);
    if (/not permitted|could not create image/i.test(err)) {
      die(
        "screencapture was refused by macOS.\n" +
          "  Screen Recording permission is required for whatever is running this\n" +
          "  (System Settings > Privacy & Security > Screen & System Audio Recording).\n" +
          "  The host app must be relaunched after granting it.",
      );
    }
    die(`screencapture failed: ${err}`);
  }
  if (!fs.existsSync(file)) die("screencapture produced no file");
